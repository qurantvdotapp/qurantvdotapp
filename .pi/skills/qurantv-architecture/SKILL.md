...
- **EXACT per-ayah bands** come from the KSU hilites API: `interface.php?ui=pc&do=hilites&mosshaf=<hafs|warsh|tajweed>&t=28&page=<p>` → `{"p": {"s_a": [x, y], ...}}`. The x/y values are in the page image's NATIVE pixel space (verified empirically: segmented the rendered page into lines — the API y-values land exactly on the matching lines, e.g. tajweed page 9: 136/222/351/648 → lines 3/5/8/15). `KsuHilitesRepository` fetches + caches forever (JsonDiskCache `ksu_hilites`); `KsuHiliteBands.build(yMap, imageHeight)` → fractional bands [y_k, y_{k+1}) (last ayah to page bottom). `PageAyahEstimator` (text-length estimate) is only the offline/fallback path.
