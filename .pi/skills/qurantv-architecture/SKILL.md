...
- **Third source — KSU (Ayat) raster pages** (styles “آيات حفص”/“آيات ورش”/“حفص ملون”): `ayat/safahat1/{p}.png` (456×672), `warsh/{p}.png` (620×1005), `tajweed_png/{p}.png` (456×707). Hafs == standard Madinah pagination (timing `page` field); Warsh and Tajweed use their own bundled paginations (`KsuWarshPageData`, `KsuTajweedPageData` — generated from quran.ksu.edu.sa quran-data.js, Tanzil-sourced page facts). No coordinate data → `PageAyahEstimator` (domain, unit-tested) estimates each ayah's band as its text length / the page's total text length (Madinah pages lay text out proportionally); surah-start pages reserve the header (~2/15); bands are FRACTIONS of the page height (`PageModeView.bandsFractional`).
