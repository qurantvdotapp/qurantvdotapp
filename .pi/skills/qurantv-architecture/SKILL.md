...
| D4 | Core scope only: Reciters → Surahs → Player + highlight. No radio/live TV; **simplified tafseer / word meanings / translation IS in scope** via the player's side-view selector (KSU Ayat data) |
...
ui/
  ...
  player/                  PlayerScreen, PlayerViewModel, TextModeList, TransportBar (3-zone layout, no seek bar),
                            SurahContentView (side-panel content list w/ auto-scroll; empty rows skipped),
                            SideContextPanel (mode header + SurahContentView; rows start at the spine),
                            SingleMushafPage (the current highlighted page for the split view; crossfades),
                            MushafSpreadView (two-page spread + page-turn animation; MushafSpine + SpineEdgeShadow shared with the split view),
...
- Page-turn: the page view is a TWO-PAGE SPREAD (odd page right, even page left — the same `page % 2` rule the KSU site uses), forced RTL so it never flips; `MushafSpreadView` animates spread changes as a page turn via AnimatedContent keyed on the spread's right-page number — **NEXT page: the new spread slides in from the LEFT, the old slides out to the RIGHT** (reversed from the original), backwards is the mirror — with a folded SPINE ribbon + inner-edge shadows. Both pages ALIGNED toward the spine (`PageModeView.alignment`: right CenterEnd, left CenterStart). The spine + edge shadow are SHARED composables (`MushafSpine()`, `BoxScope.SpineEdgeShadow(alignment)`) also used by the split view (context panel open) so both modes look like the same open mushaf. The spread loader (`loadSpreadSide`) consolidates all page sources with the highlight only on the current-ayah side + next-spread prefetch.
...
- Page-mode chrome is a TRANSLUCENT OVERLAY (0.62-alpha scrim) over the always-full-screen mushaf, auto-hiding **5 s** after the LAST key press while playing (countdown resets on every press via `lastKeyPress`); any key reveals it; pause reveals it; INFO/MENU toggles text/page mode. The auto-hide eye toggle (persisted `autoHideControls`) disables it entirely.
...
- **Side view selector — the mushaf page is ALWAYS on screen** (TRANSPORT-bar button in the right zone immediately LEFT of the display-mode/mushaf button, label shows the current selection → chooser with ✓ on current and a hint, "صفحة المصحف معروضة دائمًا — اختر ما يظهر بجانبها"): `صفحة المصحف فقط` (mushaf only — the full two-page spread fills the screen, no panel) · `التفسير الميسر` · `معاني الكلمات` · `الترجمة`. Picking a context view opens an OPTIONAL SIDE PANEL beside the always-visible mushaf, laid out like an OPEN MUSHAF (50/50 halves): the current highlighted page on the RIGHT aligned toward the spine (`SingleMushafPage` → `PageModeView(alignment = CenterEnd)`; the page is picked from the loaded spread by parity — odd = right side, even = left side — and crossfades on page change), the folded `MushafSpine()` ribbon between, and the context panel in the LEFT page's place with its rows starting at the spine (RTL `start = 14dp` inner vs `26dp` outer — `SideContextPanel` = mode header + `SurahContentView` with `contentPadding`/`fontSizeSp`/`rowSpacing` params tuned for the half width). The panel shows the WHOLE SURAH, one ayah per row: current ayah highlighted + AUTO-SCROLLS with the recitation when timing exists (static without). Rows with EMPTY content are hidden (2,576 of the 6,236 word-meaning rows are empty in the KSU DB — e.g. surah 2 ayahs 1/3/4/6/8…); when the current ayah itself has no row, the list pins to the NEAREST PRECEDING row so it still follows the recitation. `SurahContentView`'s pin/proportional-follow effects look rows up by TIMING INDEX (skipped rows shift list positions). Data from `TafseerRepository.surahContent(surahId, mode)` (bundled SQLite DBs).
...
- `AppSettings`: language (ar/en), defaultSpeed, fontSizeIndex (0..2), highlightColorIndex (0..2), displayMode (0=text, 1=page — the DEFAULT, page mode), mushafStyle (0..5 — default **5 = حفص ملون** Hafs Tajweed), ayahOffset, autoHideControls, onlyTimedReciters. (A transient `showTafseer` setting was tried then REMOVED — the side view is chosen in the player's top-bar selector instead; the choice itself is session-local, not persisted.)
