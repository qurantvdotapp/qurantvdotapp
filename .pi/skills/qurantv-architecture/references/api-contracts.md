...
| **quran.ksu.edu.sa (Ayat KSU — the reference app)** | Hafs: `https://quran.ksu.edu.sa/ayat/safahat1/{page}.png` (456×672) · Warsh: `https://quran.ksu.edu.sa/warsh/{page}.png` (620×1005) | ✅ USED (styles “آيات حفص”/“آيات ورش”) | Hafs pagination == standard Madinah == timing `page` field (verified 2:1→2, 2:6→3, 2:255→42, 50:1→518). Warsh has its own pagination — page boundaries bundled in `KsuWarshPageData` (from quran.ksu.edu.sa quran-data.js, Tanzil-sourced page facts; binary-search `warshPageFor`). Raster PNGs → no per-ayah coordinates → bottom ayah-text strip highlight + page-level sync only. |
