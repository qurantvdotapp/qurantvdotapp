...
| 2026-08-08 | 5+ | **Timing verification + Home auto-scroll fix**. (1) Confirmed the read-135 timings really belong to this reciter's audio: the read's `folder_url` == the moshaf `server` exactly; the compression is per-surah (1.12–1.22, varies) so it is a systematic mp3quran generation quirk; and 8 consecutive real-playback ayah transitions (37–40 and 58–61) align within 0.1–0.3 s of the corrected timing windows, with the last ayah's corrected end mapping to the file end — a different surah/reciter could not align at all. (2) Home no longer auto-scrolls to the top during playback: `HomeScreen`'s initial-focus LaunchedEffect was keyed on `ui.lastSession`, which updates every ~5 s while playing (session save loop), so it re-requested focus on the Continue card and yanked the list to the top; it is now keyed on `ui.lastSession?.surahId` (the session's identity) so content updates don't re-trigger it. Verified on the emulator: with السويّد الكهف playing, scrolled to the و group, waited 8 s — the list stayed put. 46/46 tests + lint green | done |
